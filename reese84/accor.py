...
requests = requests.Session(impersonate='chrome136')
...
response = requests.get(response.headers['location'], params=params, headers=headers)

src = re.findall(r'scriptElement.src = "(.*?)";', response.text)[0]
print(f"https://login.accor.com{src}")

payload = {
    "task_type": "reese84", 
    "proxy": PROXY, 
    "reese84_js_url": f"https://login.accor.com{src}",
}
